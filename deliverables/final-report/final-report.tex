...
\pagebreak
\begin{abstract}
\noindent This document concludes the our whole run-through of the \textit{Contextproject}; the development of our product: Zelula. This software application provides synthetic biologists with a visual, easy to use tool to design, simulate and validate biological logic circuits.

The project was roughly divided in two parts: the design stage and the implementation stage. Both were roughly of equal length. During the implementation the SCRUM method was employed and development was driven by tests.

The teamwork and development issues we encountered are reflected upon and analyzed.
Despite the obstacles all requirements were met and we managed to deliver a well refined product.\end{abstract}
\pagebreak
...
The first half of the project was focused on design, while the second part was all about implementation. In this chapter we will reflect on the translation from design to implementation. We will clarify what was implemented according to the documents and what was changed or perhaps not implemented.
...
Implementation was greatly accelerated by the use of frameworks and libraries such as jQuery, Bootstrap, HighCharts and jsPlumb. The use of such frameworks of course also implies the study of their use, and solving problems or limitations you encountered with them. This costed considerable time as eluded  
\newpage
