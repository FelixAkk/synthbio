...
If a component that is earlier in the critical path is not yet finished than a component further down in the critical path or not in the critical path will be delayed or cancelled. For example if Design Circuit\footnote{Requirement X} is not yet finished than work on pro\footnote{Requirement X} must be delayed because Specify Protein needs Design Circuit to be implemented.\\
